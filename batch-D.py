...
#data = np.loadtxt('dataset2.txt', delimiter=',', usecols=range(3))
...
data = np.column_stack((x_input1, x_input2))
data = np.column_stack((data, y_output))
...
for i in myx:
    j=i
    a1 = (theta[2] * j[0]) + (theta[1]*j[1]) + (theta[0])
    print(a1)
